...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for s in sents:
            curr_sent = []
            for w in s:
                curr_sent.append([self.char2id[char] for char in w if char in self.char2id ])
            embeddings.append(curr_sent)
        return embeddings
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_t = pad_sents_char(sents_ind, self.char2id['<pad>'])
        sents_var = torch.tensor(sents_t, dtype = torch.long, device = device)
        return sents_var.permute(1, 0, 2).view(max_sent_len, batch_size, max_word_length)
